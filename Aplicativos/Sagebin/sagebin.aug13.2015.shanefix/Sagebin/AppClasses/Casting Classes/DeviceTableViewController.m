...
- (ChromecastDeviceController *)castDeviceController {
  return APPDELEGATE.chromecastDeviceController;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  if (self.castDeviceController.isConnected == NO)
  {
    ...
    // Configure the cell...
    //GCKDevice *device = [self.castDeviceController.deviceFilter.devices objectAtIndex:indexPath.row];
  ...
  {
      //connected but not playing any movie
    if (indexPath.row == 0)
    {
      cell =
  ...
  else
  {
    ...
    if (indexPath.row == 0)
    {
      ...
      if (self.castDeviceController.mediaInformation.metadata.images.count > 0) {
          ...
          UIImage *image =
          [UIImage imageWithData:[NSData dataWithContentsOfURL:mediaImage.URL]];
...
- (IBAction)disconnectDevice:(id)sender {
  [self.castDeviceController disconnectFromDevice];
    //APPDELEGATE.currentVideoObj = nil;
    //[APPDELEGATE removeOldPrefrencesForKey:keyCastVideo];
