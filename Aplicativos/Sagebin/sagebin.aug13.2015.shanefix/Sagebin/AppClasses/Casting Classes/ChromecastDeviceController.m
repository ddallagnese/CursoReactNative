...
//static NSString *const kReceiverAppID = @"CFCF6970"; // @"4F8B3483";  //Replace with your app id
...
- (id)init {
  return [self initWithFeatures:ChromecastControllerFeatureHWVolumeControl];
}
...
- (void)connectToDevice:(GCKDevice *)device {
  ...
  chromecastButton.imageView.animationImages =
      @[ [UIImage imageNamed:@"icon_cast_on0"], [UIImage imageNamed:@"icon_cast_on1"],
          [UIImage imageNamed:@"icon_cast_on2"], [UIImage imageNamed:@"icon_cast_on1"] ];
...
[self deviceDisconnected]; // edited
    [self updateCastIconButtonStates]; // edited
}
...
- (void)deviceDidComeOnline:(GCKDevice *)device forDeviceFilter:(GCKDeviceScanner *)deviceFilter {
...
- (void)deviceDidGoOffline:(GCKDevice *)device forDeviceFilter:(GCKDeviceScanner *)deviceFilter {
    [self updateCastIconButtonStates];
}
...
- (BOOL)loadMedia:(NSURL *)url
    thumbnailPath:(NSString *)posterPath
           ...
           strURL:(NSString *)strURL
{
  ...
  if (posterPath) {
      [metadata addImage:[[GCKImage alloc]
                          initWithURL:[[NSURL alloc] initWithString:posterPath]
                          width:200
                          height:200]];
  }

  GCKMediaInformation *mediaInformation =
      [[GCKMediaInformation alloc] initWithContentID:strURL
...
//  _btnImageConnected = [UIImage imageNamed:@"cast_solid_custom.png"];
    _btnImage = [UIImage imageNamed:@"new_icon_cast_off"];
    _btnImageConnected = [UIImage imageNamed:@"cast_solid_custom"];
  ...
  chromecastButton.hidden = YES;
    [chromecastButton setAutoresizingMask:UIViewAutoresizingFlexibleLeftMargin];  
...
- (void)updateToolbarStateIn:(UIViewController *)viewController {
  ...
  if (self.isPlayingMedia) {
    viewController.navigationController.toolbarHidden = YES; // edited 26Aug
  } else {
  ...
  //Loading thumbnail async
    ...
    self.toolbarThumbnailImage.image =[UIImage imageWithData:[NSData dataWithContentsOfURL:img.URL]];
    /*
